...
| `../../.github/workflows/upstream-radar.yml` | The weekly upstream radar (Mondays 05:37 UTC): compares the revisions recorded in `UPSTREAM_SYNC.md` with the code upstream's latest release tag and default-branch head and the reference upstream's default-branch head, then opens or updates exactly one `Upstream update available` issue when something moved. Read-only — it never merges, pulls, pushes or writes to the repository. |
